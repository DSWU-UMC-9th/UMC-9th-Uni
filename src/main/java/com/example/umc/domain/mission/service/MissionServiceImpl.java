...
public class MissionServiceImpl implements MissionService {
    ...
    // 진행 중인 미션 리스트
    public MissionResDTO.MyMissionListDTO getMyMissionPage(Long memberId, Integer page) {        int pageIndex = (page == null || page < 1) ? 0 : page - 1;
        PageRequest pageRequest = PageRequest.of(pageIndex, 5);

        Page<MemberMission> inProgress = memberMissionRepository
                .findAllByMemberIdAndStatus(memberId, MissionStatus.PROGRESSING, pageRequest);

        Page<MemberMission> completed = memberMissionRepository
                .findAllByMemberIdAndStatus(memberId, MissionStatus.SUCCESS, pageRequest);

        return MissionConverter.toMyMissionListDTO(inProgress, completed);
    }
